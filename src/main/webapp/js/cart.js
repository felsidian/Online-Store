...
document.querySelectorAll('.add-to-cart').forEach(item => {
    item.addEventListener('click', event => {
        ...
        CartStorage.addToCart(productStrings[0], productStrings[1], productStrings[2], productStrings[3]);
        updateCartSizeEl();
    })
});
...
function fillCart() {
    ...
    for (let id of Object.keys(cart)) {
        ...
        newElement.innerHTML = createCartItemElement(id, product.name, product.price, product.quantity, product.imageUrl);
...
function createCartItemElement(id, name, price, quantity, imageUrl) {
    return [
        '<div class="row d-flex align-items-center">',
        '<div class="col-lg-3 col-4">',
        '<img class="card-img-top p-3" src="'+ imageUrl +'" style="height: 100px; object-fit: scale-down;" alt="">',
        '</div>',
